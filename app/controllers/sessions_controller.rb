...
class SessionsController < ApplicationController
  ...
  def create
    ...
    #проверка создания нового пользователя (если не создали то обновляем)
      if user.persisted?
        user.update(access_token: accessToken, refresh_token: refreshToken, access_token_expiration_time: Time.now + 55.minutes, user_device_id: deviceId)
      end
      sign_in user
  ...
  def view
    if user_signed_in?
      if needs_refresh_token?
        ...
        user.update(access_token: response_from_refresh['access_token'], refresh_token: response_from_refresh['refresh_token'], access_token_expiration_time: Time.now + 55.minutes)
      end
